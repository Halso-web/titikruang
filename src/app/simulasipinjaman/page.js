...
export default function KuisBintang() {
  ...
  const [pengeluaranList, setPengeluaranList] = useState([{ nama: '', jumlah: 0 }]);
  ...
  const hari = tenor * 30;
  const totalPengeluaran = pengeluaranList.reduce((acc, cur) => acc + (Number(cur.jumlah) || 0), 0);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#3061F2] via-white to-[#F2BF27]/10 text-gray-800">
      ...
      <div className="py-10 px-6">
        <h1 className="text-3xl font-bold text-center mb-8 text-[#F2780C]">Simulasi Pinjaman Harian</h1>

        <div className="max-w-3xl mx-auto space-y-6 bg-white p-6 rounded-xl shadow-xl">
          <div>
            <label className="block font-semibold mb-2">Jumlah Pinjaman (Rp)</label>
            <input
              type="range"
              min={1000000}
              max={100000000}
              ...
              className="w-full"
            />
            <p className="mt-1 text-blue-600 font-semibold">Rp {harga.toLocaleString()}</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full"
            />
            <p className="mt-1 text-blue-600 font-semibold">{tenor} bulan ({hari} hari)</p>
          </div>

          <div>
            <label className="block font-semibold mb-2">Bunga per Hari (%)</label>
            <input
              type="range"
              min={0.1}
              max={1}
              ...
              className="w-full"
            />
            <p className="mt-1 text-blue-600 font-semibold">{bunga.toFixed(1)}% per hari</p>
          </div>

          <div>
            <label className="block font-semibold mb-2">Dana Pribadi (Rp)</label>
            <input
              type="range"
              min={0}
              max={100000000}
              ...
              className="w-full"
            />
            <p className="mt-1 text-blue-600 font-semibold">Rp {danaPribadi.toLocaleString()}</p>
            <p className="text-sm text-gray-500 mt-1">*Gaji tiap Bulan</p>
          </div>

          <div>
            <label className="block font-semibold mb-2 mt-4">Rincian Pengeluaran Pribadi</label>
            {pengeluaranList.map((item, index) => (
              <div key={index} className="flex gap-2 mb-2">
                <input
                  ...
                  value={item.nama}
                  onChange={(e) => handlePengeluaranChange(index, 'nama', e.target.value)}
                ...
                <input
                  ...
                  value={item.jumlah}
                  onChange={(e) => handlePengeluaranChange(index, 'jumlah', e.target.value)}
                  className="w-40 border p-2 rounded"
                />
                {index > 0 && (
                  <button onClick={() => removePengeluaran(index)} className="text-red-600 font-bold px-2">−</button>
                )}
              </div>
            ))}
            <button onClick={addPengeluaran} className="mt-2 text-blue-600 font-semibold">+ Tambah Pengeluaran</button>
            <p className="mt-2 text-sm text-gray-600">Total Pengeluaran: Rp {totalPengeluaran.toLocaleString()}</p>
            <p className="text-sm text-gray-600">Sisa Dana Bersih: Rp {sisaDana.toLocaleString()}</p>
          </div>

          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h2 className="text-lg font-bold text-blue-700">Hasil Simulasi</h2>
            <p className="mt-2">Jumlah Pinjaman: <strong>Rp {pinjaman.toLocaleString()}</strong></p>
            <p>Total Bunga ({hari} hari): <strong>Rp {Math.round(bungaTotal).toLocaleString()}</strong></p>
            <p>Total yang Harus Dibayar: <strong className="text-red-600 text-xl">Rp {Math.round(totalPembayaran).toLocaleString()}</strong></p>
            <p>Pembayaran Per Bulan: <strong className="text-blue-600 text-xl"> Rp {Math.round(pembayaranPerBulan).toLocaleString()}</strong></p>
          </div>

          <div className="mt-4 p-4 bg-white border rounded-lg">
            <h3 className="font-semibold text-gray-800 mb-2">Evaluasi Kemampuan Dana Pribadi</h3>
            {evaluasiSisa >= 0 ? (
              <p className="text-green-600 font-semibold">
                ✅ Dana kamu cukup untuk membayar tiap bulan. Sisa bulanan: Rp {Math.round(evaluasiSisa).toLocaleString()} Apa Kamu yakin Masih ingin Meminjam?. Coba kamu Pertimbangkan lagi
              </p>
            ) : (
              <p className="text-red-600 font-semibold">
                ❌ Dana kamu tidak memenuhi untuk melakukan pembayaran tiap bulannya. uang yang masih kamu perlu bayarkan: Rp {Math.abs(Math.round(evaluasiSisa)).toLocaleString()} apa kamu yakin masih ingin meminjam?
              </p>
            )}
          </div>
        </div>
      ...
      <footer className="text-white bg-gradient-to-r from-[#3061F2] via-[#27A4F2] to-[#F2780C] relative pt-0">
        <svg className="w-full h-20 md:h-28 block" viewBox="0 0 1440 320" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
          <path fill="M0,64 C480,160 960,0 1440,96 L1440,0 L0,0 Z" />
        </svg>
